...
ตอบ เพราะจะช่วยให้ โค้ดเป็นระเบียบ, เข้าใจง่าย, และ แก้ไขง่าย เวลามีหลายฟังก์ชันหรือโมดูล เช่น LED, Sensor, Display แยกออกจากกัน จะไม่ปะปนกัน และทีมงานสามารถทำงานพร้อมกันได้ง่ายขึ้น

2.CMakeLists.txt Management: การเพิ่มไฟล์ source ใหม่ต้องแก้ไขอะไรบ้าง?<br>
ตอบ ต้องไปเพิ่มชื่อไฟล์ .c หรือ .cpp ที่เราสร้างใหม่ เข้าใน CMakeLists.txt เพื่อให้ระบบรู้ว่าไฟล์นี้ต้องนำไป compile ด้วย

3.Header Files: บทบาทของไฟล์ .h คืออะไร และทำไมต้องมี?<br>
ตอบ ไฟล์ .h คือไฟล์ที่เอาไว้ ประกาศฟังก์ชัน, ตัวแปร, และโครงสร้างข้อมูล เพื่อให้ไฟล์อื่นเรียกใช้งานได้

4.Include Directories: เหตุใด CMakeLists.txt ต้องระบุ INCLUDE_DIRS?<br>
ตอบ เพื่อบอกว่า ไฟล์ .h อยู่ที่ไหน เวลาคอมไพล์จะได้หาเจอ เช่นถ้าเราเก็บ header ไว้ในโฟลเดอร์ include/ ต้องใส่ไว้ใน INCLUDE_DIRS ไม่งั้นจะ error ว่า "หาไฟล์ไม่เจอ"

5.Git Ignore: ไฟล์ .gitignore ช่วยอะไรในการจัดการ ESP32 project?<br>
ตอบ มันช่วยให้ ไม่เก็บไฟล์ที่ไม่จำเป็นลง Git เช่นไฟล์ชั่วคราว, ไฟล์ build, หรือไฟล์ที่ระบบสร้างอัตโนมัติ ซึ่งจะช่วยให้ project สะอาด และแชร์เฉพาะไฟล์ที่จำเป็นเท่านั้น

6.Task Management: การใช้ FreeRTOS task ในโมดูล LED ช่วยอะไร?<br>
ตอบ ช่วยให้ จัดการ LED แบบแยกอิสระ จากงานอื่น เช่นกระพริบ LED ไปพร้อมกับอ่าน sensor หรือแสดงผล display ได้ในเวลาเดียวกัน โดยไม่ต้องรอคิวกัน

7.Code Organization: ข้อดีของการแยกโมดูล sensor, display, led เป็นไฟล์แยกคืออะไร?<br>
ตอบ โค้ด เป็นระเบียบ, หาและแก้ไขง่าย, นำกลับมาใช้ซ้ำได้, ทำงานเป็นทีมได้ง่าย แต่ละคนดูแลคนละไฟล์
